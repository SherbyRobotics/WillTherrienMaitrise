...
x_aug = X[:,0] / X[:,1] * np.tan(X[:,3])
X_aug = np.append(X,x_aug[:,np.newaxis], 1)
...
x_test_aug = X_test[:,0] / X_test[:,1] * np.tan(X_test[:,3])
X_test_aug = np.append(X_test,x_test_aug[:,np.newaxis], 1)

Y_hat_test = model.predict(X_test_aug)
